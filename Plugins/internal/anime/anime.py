...
async def anime(update: Update, context):
    ...
    try: 
        if json_data:
            ...
            authors = anime_data.get("staff", [])
            author_names = [author["name"]["full"] for author in authors]
            ...
            if author_names:
                msg += f"🖋 *Authors:* {', '.join(author_names)}\n"
        ...
        await message.reply_text("Enter a proper name, I'cant fetch info about it. Try again.")
